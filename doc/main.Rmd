...
bind_pos_neg_polarity  = tidy_format_bad %>%
  ...
  group_by(document) %>%
  summarise(summary_polarity = sum(polarity)) %>%
...
To be cont'd: "good" listings are more emotional.
...
```{r}
polar_data_full %>% select(id, full_text, category) %>%
group_by(category) %$% polarity(full_text)
```
